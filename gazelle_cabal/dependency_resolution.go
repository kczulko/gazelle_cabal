...
func getPackageLabel(
    ...
    for _, r := range res {
		// prevent circular dependency, in such case we take dep from package repository
		if (r.IsSelfImport(from)) {
			return generateRepositoryDepLabel(packageRepo, packageName, from)
		}
...
func generateRepositoryDepLabel(packageRepo string, packageName string, from label.Label) label.Label {
